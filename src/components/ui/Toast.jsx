...
function Toast({ message, type = 'info', onClose }) {
  ...
  return (
    <div
      className={`pointer-events-auto flex items-center gap-3 px-4 py-3 rounded-lg border shadow-lg transition-all duration-200 ${config.bg} ${config.border} ${
    ...
    >
      ...
      <button
        onClick={handleClose}
        className={`flex-shrink-0 p-1 rounded hover:bg-black/5 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-${type}-500`}
        aria-label="Close notification"
      >
        <svg className={`w-4 h-4 ${config.text}`} fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
...
export const toast = {
  success: (message, duration) => {
    // This requires ToastProvider context
